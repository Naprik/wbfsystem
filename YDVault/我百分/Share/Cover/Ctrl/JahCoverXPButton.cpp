...
CJahCoverXpButton::CJahCoverXpButton()
{
	m_BoundryPen.CreatePen(PS_INSIDEFRAME | PS_SOLID, 1, RGB(0, 0, 0));
	m_InsideBoundryPenLeft.CreatePen(PS_INSIDEFRAME | PS_SOLID, 3, RGB(221, 170, 170)); 
	m_InsideBoundryPenRight.CreatePen(PS_INSIDEFRAME | PS_SOLID, 3, RGB(238, 184, 184));
	m_InsideBoundryPenTop.CreatePen(PS_INSIDEFRAME | PS_SOLID, 2, RGB(194, 185, 185));
	m_InsideBoundryPenBottom.CreatePen(PS_INSIDEFRAME | PS_SOLID, 2, RGB(200, 190, 190));
...
BEGIN_MESSAGE_MAP(CJahCoverXpButton, CButton)
	//{{AFX_MSG_MAP(CJahCoverXpButton)
	ON_WM_MOUSEMOVE()
...
/////////////////////////////////////////////////////////////////////////////
// CJahCoverXpButton message handlers

//--------添加Owner Draw属性--------------------------------------

//##ModelId=4140A36802A3
void CJahCoverXpButton::PreSubclassWindow() 
{
...
void CJahCoverXpButton::OnMouseMove(UINT nFlags, CPoint point) 
{
...
LRESULT CJahCoverXpButton::OnMouseLeave(WPARAM wParam, LPARAM lParam)
{
	m_bOver = FALSE;
	m_bTracking = FALSE;
...
LRESULT CJahCoverXpButton::OnMouseHover(WPARAM wParam, LPARAM lParam)
{
	m_bOver = TRUE;
	InvalidateRect(NULL);
	return 0;
...
void CJahCoverXpButton::DrawItem(LPDRAWITEMSTRUCT lpDrawItemStruct)
{
...
void CJahCoverXpButton::DoGradientFill(CDC *pDC, CRect* rect)
{
	CBrush brBk[64];
...
void CJahCoverXpButton::DrawInsideBorder(CDC *pDC, CRect* rect)
{
	...
	else
	{
		...
		m_BrushColor = RGB(170, 213, 255);   //鼠标移到按钮上显示的颜色
	}
...
//---------鼠标移到按钮上的底色控制--------------------------------
	...
	m_OldPen = (CPen * )pDC->SelectObject(m_Pen);
	m_Oldbrush = (CBrush *)pDC->SelectObject(m_brush);
	pDC->Rectangle(rect);
