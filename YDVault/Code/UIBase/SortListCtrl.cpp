...
bool CSortListCtrl::IsPercent(LPCTSTR pszText)
{
	...
	if(str.Find(_T("%")) >= 0 && str.Find(_T("%")) == str.GetLength()-1)
	{
		return true;
	}
	return false;
...
bool CSortListCtrl::IsStr(LPCTSTR pszText)
{
	ASSERT_VALID_STRING( pszText );
	CString str =pszText ;
	if (str.GetLength() >= 0)
...
int CSortListCtrl::StrCompare(LPCTSTR strPer1, LPCTSTR strPer2)
{
	CString str1 = strPer1;
	CString str2 = strPer2;
	if (str1 > str2)
	{
		return 1;
	}
...
int  CSortListCtrl::PercentCompare(LPCTSTR strPer1, LPCTSTR strPer2)
{
	...
	CString s1 (cStr1, cStr1.Find(_T("%"))); 
	CString s2 (cStr2, cStr2.Find(_T("%"))); 
	int i1 = _ttoi(s1);
	int i2 = _ttoi(s2);
	if(i1 < i2)
		return -1;
...
int CALLBACK CSortListCtrl::CompareFunction( LPARAM lParam1, LPARAM lParam2, LPARAM lParamData )
{
	...
	else if (IsStr(pszText1))
		return pListCtrl->m_bSortAscending ? StrCompare( pszText2, pszText1 ) : StrCompare( pszText1, pszText2 );
	else
