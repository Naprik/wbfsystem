...
void cdxCSizingDialog::ControlInfo::Header::StretchWindow(UINT addSzByPcnt)
{
	...
	if(!m_pWnd->GetWindowPlacement(&wpl))
	{
		...
		CString strTrace;
		strTrace.Format(_T("*** ERROR[cdxCSizingDialog::ControlInfo::Header::StretchWindow()]: GetWindowPlacement() failed for dialog with caption \"%s\" !\n"),s);
		TRACE(strTrace);
#endif
		return;
	}
...
cdxCSizingDialog::ControlInfo *cdxCSizingDialog::ControlInfo::Header::AddSzControlEx(CWnd & ctrl, BYTE dX1pcnt, BYTE dX2pcnt, BYTE dY1pcnt, BYTE dY2pcnt)
{
		...
		(dY1pcnt == exIgnore) && (dY2pcnt == exIgnore) )
	{
		...
		TRACE(_T("*** WARNING[cdxCSizingDialog::ControlInfo::Header::AddSzControl()]: For control \"%s\", all modifcators are set to NONE (control will not be added).\n"),(LPCTSTR)s);
#endif
		return NULL;
	}
...
BOOL cdxCSizingPropSheet::OnInitDialog(UINT addSzByPcnt) 
{
	...
	for(i=0; psheetCtrls[i].id != BAD_ID; ++i)
	{
		...
		{
			TRACE(_T("*** WARNING[cdxCSizingPropSheet::OnInitDialog()]: Can't find CWnd for ID#%ld !\n"),psheetCtrls[i].id);
		}
	}
