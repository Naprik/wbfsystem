// YDCompositionQuestionDlg.cpp : implementation file
//
...
IMPLEMENT_DYNAMIC(CYDCompositionQuestionDlg, CYDQuestionDlg)

CYDCompositionQuestionDlg::CYDCompositionQuestionDlg(CWnd* pParent /*=NULL*/)
	: CYDQuestionDlg(CYDCompositionQuestionDlg::IDD, pParent)
	, m_strArticle(_T(""))
	, m_strAnswer(_T(""))
	, m_iHardLevel(0)
	, m_strCreateDate(_T(""))
...
CYDCompositionQuestionDlg::~CYDCompositionQuestionDlg()
{
	CListAutoClean<CYdObjWrapper> clean1(m_lstAddKPs);
}

void CYDCompositionQuestionDlg::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(CYDCompositionQuestionDlg, CYDQuestionDlg)
	ON_BN_CLICKED(IDC_BUTTON_CQ_ADD, &CYDCompositionQuestionDlg::OnBnClickedButtonCqAdd)
	ON_BN_CLICKED(IDC_BUTTON_CQ_MODIFY, &CYDCompositionQuestionDlg::OnBnClickedButtonCqModify)
	ON_BN_CLICKED(IDC_BUTTON_ADDKP, &CYDCompositionQuestionDlg::OnBnClickedButtonAddkp)
	ON_BN_CLICKED(IDC_BUTTON_REMOVEKP, &CYDCompositionQuestionDlg::OnBnClickedButtonRemovekp)
	ON_BN_CLICKED(IDCANCEL, &CYDCompositionQuestionDlg::OnBnClickedCancel)
END_MESSAGE_MAP()


// CYDCompositionQuestionDlg message handlers

void CYDCompositionQuestionDlg::OnBnClickedButtonCqAdd()
{
...
void CYDCompositionQuestionDlg::OnBnClickedButtonCqModify()
{
	// TODO: Add your control notification handler code here
	CListAutoClean<CYdObjWrapper> clean1(m_lstAddKPs);
	UpdateData();
...
BOOL CYDCompositionQuestionDlg::OnInitDialog()
{
...
BOOL CYDCompositionQuestionDlg::ValidateData(OPERATION op)
{
	if(m_strArticle.IsEmpty())
	{
		AfxMessageBox(_T("文摘不能为空！"),MB_OK|MB_ICONINFORMATION);
		return FALSE;
	}
...
HRESULT CYDCompositionQuestionDlg::CreateQuestionRef(CYDArticleQuestionRef* &_pRef)
{
	HRESULT hr = E_FAIL;
	ASSERT(_pRef == NULL);
	CDatabaseEx* pDB = (CDatabaseEx*)AfxGetMainWnd()->SendMessage(WM_YD_GET_DB);
...
HRESULT CYDCompositionQuestionDlg::CreateLinkRef(CYDLinkRef* &_pLinkRef)
{
	HRESULT hr = E_FAIL;
	CDatabaseEx* pDB = (CDatabaseEx*)AfxGetMainWnd()->SendMessage(WM_YD_GET_DB);
	ASSERT(pDB);
...
HRESULT CYDCompositionQuestionDlg::UpdateQuestionArea()
{
	HRESULT hr = E_FAIL;
	if (m_uType == OP_NEW)
	{
...
HRESULT CYDCompositionQuestionDlg::EnableQuestionArea(BOOL bEnable)
{
	HRESULT hr = E_FAIL;
...
HRESULT CYDCompositionQuestionDlg::UpdateQuestionRef(CYDArticleQuestionRef* _pRef)
{
	HRESULT hr = E_FAIL;
...
BOOL CYDCompositionQuestionDlg::IfNewKPRemoveFromCache(CYdObjWrapper* pObjWrapper)
{
...
HRESULT CYDCompositionQuestionDlg::InitKnowledge()
{
	HRESULT hr = E_FAIL;
	CRect rectKP;
...
void CYDCompositionQuestionDlg::OnBnClickedButtonAddkp()
{
	// TODO: Add your control notification handler code here
	HRESULT hr = E_FAIL;
...
void CYDCompositionQuestionDlg::OnBnClickedButtonRemovekp()
{
	// TODO: Add your control notification handler code here
...
HRESULT CYDCompositionQuestionDlg::PersistKnowledge(CYDObjectRef* _pParentRef)
{
	HRESULT hr = E_FAIL;
...
void CYDCompositionQuestionDlg::OnBnClickedCancel()
{
	// TODO: Add your control notification handler code here
	OnCancel();
}

INT_PTR CYDCompositionQuestionDlg::DoModal()
{
